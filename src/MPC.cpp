...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The cost is stored in thef irst element of `fg`
    fg[0] = 0;
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 250*CppAD::pow((vars[v_start + t] * vars[delta_start + t]), 2);
...
std::vector<double> MPC::Simulate(std::vector<double> state, std::vector<double> actuations, double latency) {
    double x0 = state[0];
    double y0 = state[1];
    double psi0 = state[2];
    double v0 = state[3];
    double a = actuations[0];
    double delta = actuations[1];

    double x1 = x0 + ( v0 * cos(psi0) * dt );
    double y1 = y0 + ( v0 * sin(psi0) * dt );
    double psi1 = psi0 - ( v0 * delta * dt / Lf );
    double v1 = v0 + a * dt;

    std::vector<double> state1 = {x1, y1, psi1, v1};
    return state1;
